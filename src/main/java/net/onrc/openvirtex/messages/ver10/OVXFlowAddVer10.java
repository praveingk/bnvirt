...
public class OVXFlowAddVer10 extends OFFlowAddVer10 implements OVXFlowAdd {



    protected OVXFlowAddVer10(
			long xid,
			Match match,
			U64 cookie,
			int idleTimeout,
	...
	private final Logger log = LogManager.getLogger(OVXFlowAddVer10.class.getName());
    ...
    public void devirtualize(final OVXSwitch sw) {
        ...
        OFMatchV1 origMatch = (OFMatchV1) this.match.createBuilder().build();
        ...
        if (ovxInPort == null) {
            ...
            for (short iport : sw.getPorts().keySet()) {
                ...
                int wcard = origMatch.getWildcards()
                        & (~OVXMatch.OFPFW_IN_PORT);
                OFMatchV1 myMatch  = origMatch.createBuilder().setInPort(OFPort.of(iport)).setWildcards(wcard).build();
                myNewFlow = OVXFactoryInst.myOVXFactory.buildOVXFlowAdd(this.getXid(), myMatch, this.getCookie(), this.getIdleTimeout(), this.getHardTimeout(), this.getPriority(), this.getBufferId(), this.getOutPort(), this.getFlags(), origActions);
    ...
    private void prepAndSendSouth(OVXPort inPort, boolean pflag) {
        ...
        try {
            if (inPort.isEdge()) {
                ...
                if (GlobalConfig.bnvTagType == TAG.TOS) {
                    TenantMapperTos.rewriteMatch(sw.getTenantId(), (OFMatchV1) this.match);
                } else if (GlobalConfig.bnvTagType == TAG.VLAN) {
                    TenantMapperVlan.rewriteMatch(sw.getTenantId(), (OFMatchV1) this.match);
                } else if (GlobalConfig.bnvTagType == TAG.IP) {
