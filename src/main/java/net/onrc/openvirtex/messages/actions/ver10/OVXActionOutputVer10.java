...
public class OVXActionOutputVer10 extends OFActionOutputVer10 implements
OVXActionOutput {
    public OVXActionOutputVer10(OFPort port, int maxLen) {
		super(port, maxLen);
		// TODO Auto-generated constructor stub
	}

	Logger log = LogManager.getLogger(OVXActionOutputVer10.class.getName());
    ...
    public void virtualize(final OVXSwitch sw,
        ...
        final OVXPort inPort = sw.getPort(match.getInPort().getShortPortNumber());

        // TODO: handle TABLE output port here

        final LinkedList<OVXPort> outPortList = this.fillPortList(
                match.getInPort().getShortPortNumber(), this.getPort().getShortPortNumber(), sw);
        ...
        if (match.isFlowMod()) {
            ...
            if (fm.getCommand().equals(OFFlowModCommand.ADD))
            	fm= OVXFactoryInst.myOVXFactory.buildOVXFlowAdd(fm.getXid(), fm.getMatch(),U64.of(match.getCookie()), fm.getIdleTimeout(), fm.getHardTimeout(), fm.getPriority(), fm.getBufferId(), fm.getOutPort(), fm.getFlags(), fm.getActions());
            else if (fm.getCommand().equals(OFFlowModCommand.DELETE))		
            	fm=OVXFactoryInst.myOVXFactory.buildOVXFlowDelete(fm.getXid(), fm.getMatch(),U64.of(match.getCookie()), fm.getIdleTimeout(), fm.getHardTimeout(), fm.getPriority(), fm.getBufferId(), fm.getOutPort(), fm.getFlags(), fm.getActions());
            else if (fm.getCommand().equals(OFFlowModCommand.DELETE_STRICT))		
            	fm=OVXFactoryInst.myOVXFactory.buildOVXFlowDeleteStrict(fm.getXid(), fm.getMatch(),U64.of(match.getCookie()), fm.getIdleTimeout(), fm.getHardTimeout(), fm.getPriority(), fm.getBufferId(), fm.getOutPort(), fm.getFlags(), fm.getActions());
            else if (fm.getCommand().equals(OFFlowModCommand.MODIFY))		
            	fm=OVXFactoryInst.myOVXFactory.buildOVXFlowModify(fm.getXid(), fm.getMatch(),U64.of(match.getCookie()), fm.getIdleTimeout(), fm.getHardTimeout(), fm.getPriority(), fm.getBufferId(), fm.getOutPort(), fm.getFlags(), fm.getActions());
                    ...
                    if (inPort.isEdge()) {
                        if (outPort.isEdge()) {
                            ...
                            if (GlobalConfig.bnvTagType == TAG.TOS) {
                                ...
                                TenantMapperVlan.prependUnRewriteActions((OFMatchV1) fm.getMatch(), approvedActions);
                            ...
                            if (link != null) {
                                try {
                                    ...
                                    for (final OFAction act : approvedActions) {
                                        if (act instanceof OVXActionDataLayerSourceVer10) {
                                            skipSrcMac = true;
                                        }
                                        if (act instanceof OVXActionDataLayerDestinationVer10) {
                                            skipDstMac = true;
                                        }
                                    }
