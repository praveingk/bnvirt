...
public class OVXPortStatusVer13 extends OFPortStatusVer13 implements OVXPortStatus {

   
    protected OVXPortStatusVer13(long xid, OFPortReason reason, OFPortDesc desc) {
		super(xid, reason, desc);
    ...
    @Override
    public void virtualize(final PhysicalSwitch sw) throws SwitchMappingException {
        Mappable map = sw.getMap();
        PhysicalPort p = sw.getPort(this.getDesc().getPortNo().getShortPortNumber());
        if (p == null) {
            handlePortAdd(sw, p);
            return;
        }
                        ...
                        if (isReason(OFPortReason.DELETE)) {
                            ...
                            if (isState(OFPortState.LINK_DOWN)) {
                                ...
                                vport.handlePortDisable(this);
                                    ...
                                    && ((OFPortStateSerializerVer13.toWireValue(p.getState()) & OFPortStateSerializerVer13.LINK_DOWN_VAL
                                            ) == 0)) {
                                /*
     ...
     */
    private void handlePortAdd(PhysicalSwitch sw, PhysicalPort p) throws SwitchMappingException {
        if (isReason(OFPortReason.ADD)) {
            p = new PhysicalPort(this.getDesc(), sw, true);
            if (!sw.addPort(p)) {
                log.warn("Could not add new port {} to physical switch {}",
                        p.getPortNo(), sw.getSwitchId());
            }
            log.info("Added port {} to switch {}", p.getPortNo(),
                    sw.getSwitchId());
        }
    }
    ...
    private void handleLinkChange(PhysicalSwitch sw, Mappable map,
        ...
        if (!isState(OFPortState.LINK_DOWN)
                && ((OFPortStateSerializerVer13.toWireValue(plink.getSrcPort().getState()) & OFPortStateSerializerVer13.LINK_DOWN_VAL) == 0)) {
    ...
    public boolean isState(OFPortState state) {
        return this.getDesc().getState().contains(state);
    }
